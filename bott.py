...
BOT_TOKEN = "7868865633:AAF4rSx3Hzv3XkgS3Ztz7Nsw9DYzZ3Z6-G4"
...
HARDCODED_ADMIN_IDS = [8344012762]
                    ...
                    try:
                        ...
                        if test_response.status_code == 200:
                            try:
                                ...
                                test_products = test_data if isinstance(test_data, list) else test_data.get("products", [])
                                if not test_products:
                                    logger.info(f"Site {shop_url} is reachable but has no products (closed/empty store)")
                                    is_proxy_issue = False
                                    is_closed_store = True
                                else:
                                    logger.info(f"Site {shop_url} has products but initial detection failed (proxy issue likely)")
                                    is_proxy_issue = True
                                    is_closed_store = False
                            except (ValueError, KeyError):
                                logger.warning(f"Site {shop_url} returned non-JSON response (bot challenge or proxy block)")
                                is_proxy_issue = True
                                is_closed_store = False
